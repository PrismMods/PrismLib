...
namespace PrismLib.UI
{
    /* Top-right toast slotting across MODS.
       ...
       card "UpdateToast", 24px inset). Sapphire's sat in exactly the same place, so with both
       mods installed the two cards drew on top of each other. The mods cannot reference each
       ...
       A mod using this follows that shape both to FIND the others and to BE findable — a mod that
       never adopts it simply stays put and the others stack under it, without either side taking a
       dependency. Living in PrismLib means every Prism mod gets the same behaviour for free.
    ...
    {
        public const string CanvasSuffix = "UpdateToastCanvas";
        public const string CardName = "UpdateToast";
        ...
        public static float OffsetFor(string selfCanvasName)
        {
